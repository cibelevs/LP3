...
public class TratadorCliente implements Runnable {
    ...
    private String tratarReserva(String[] partes) {
        ...
        /** */
        // 1. Validar número de parâmetros
        if (partes.length < 4) {
        ...
        String categoriaStr = partes[2].toUpperCase();
        String codigoAssento = partes[3];
        ...
        // 5. Criar requisição
        RequisicaoReserva req = new RequisicaoReserva(idCliente, nome, categoria, codigoAssento);

        // 6. Adicionar à fila
        filaRequisicoes.offer(req);

        try {
        // 7. Aguardar resposta com timeout
        RespostaReserva resp = req.getFilaResposta().poll(30, TimeUnit.SECONDS);
        ...
        // 9. Sucesso
        if (resp.isSucesso()) {
            return String.format("OK %s %s", resp.getCodigoAssento(), resp.getCodigoReserva());
        } else {
            // 10. Falha
            return String.format("FAIL %s %s", resp.getCodigoErro(), resp.getMensagemErro());
        }
