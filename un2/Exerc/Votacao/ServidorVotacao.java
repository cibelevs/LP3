...
public class ServidorVotacao extends UnicastRemoteObject implements ServicoVotacao {
    // ... (Variáveis de instância permanecem iguais) ...
    ...
    public ServidorVotacao() throws RemoteException {
        // ... (Construtor e inicializarDados() permanecem iguais) ...
        super();
    ...
    private void cadastrarEleitor(String cpf, String senha, boolean admin) {
        ...
        eleitores.put(cpf, new Eleitor(admin, senhaHash, cpf));
    }
    ...
    public synchronized String autenticar(String cpf, String senha, boolean admin) throws RemoteException {
        ...
        Eleitor eleitorBusca = eleitores.get(cpf);
        if(eleitorBusca == null){
            mensagem = "Falha: eleitor não encontrado";
            log(mensagem);
            return null;
        }

        String senhaH = gerarHash(senha);

        if(senhaH != eleitorBusca.getSenhaHash()){
            mensagem = "Falha: senha incorreta";
            log(mensagem);
            return null;
        }

        if(eleitorBusca.isAdmin() != admin){
            mensagem = "Falha: Tentativa de autenticação";
            log(mensagem);
            return null;
        }
        ...
        sessoes.put(token, new SessaoAutenticacao(cpf, admin));
        mensagem = "Autentificação bem sucedida";
        log(mensagem);
        return token;
    ...
    public synchronized boolean registrarVoto(String token, int idCandidato) throws RemoteException {
        ...
        String mensagem = null;
        boolean validacao = validarToken(token);
        SessaoAutenticacao sessaoBusca = sessoes.get(token);
        
        if(sessaoBusca == null || !validacao){
            mensagem = "Voto rejeitado: sessão inválida";
            log(mensagem);
            return false;
        }

        if(status != StatusEleicao.EM_ANDAMENTO){
            mensagem = "Voto rejeitado: eleição não está em andamento";
            log(mensagem);
            return false;
        }

        String cpfBusca = sessaoBusca.getCpf();
        if (cpfsQueVotaram.contains(cpfBusca)){
            mensagem = "Voto rejeitado: eleitor já votou";
            log(mensagem);
            return false;
        }

        boolean cd = candidatos.containsKey(idCandidato);
        if(!cd){
            mensagem = "Voto rejeitado: candidato inválido";
            log(mensagem);
            return false;
        }

        votos.put(idCandidato, votos.get(idCandidato + 1));
        cpfsQueVotaram.add(cpfBusca);
    ...
    // ... (Todos os outros métodos, como jaVotou, listarCandidatos, obterResultadoParcial, etc., permanecem iguais) ...
