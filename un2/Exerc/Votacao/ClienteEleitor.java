...
public class ClienteEleitor {
    ...
    private void votar() {
        try {
            ...
            boolean votacao = servico.jaVotou(token);
            if(votacao){
                System.out.println("Voce ja votou");
                return;
            }

            String status = servico.obterStatusEleicao();
            if(!"EM_ANDAMENTO".equals(status)){
               System.out.println("A eleição não está em andamento...");
               return; 
            }
            
            listarCandidatos();
            System.out.print("Insira o id do candidato: ");
            int idVoto = scanner.nextInt();

            System.out.print("Voce confirma seu voto (Digite S para SIM e N para NÃO): ");
            ...
            if (servico.registrarVoto(token, idVoto)){
